...
public class PlayerTaskManager {
    ...
    public long getNextRefreshMillis(Player player) {
        long next = Long.MAX_VALUE;
        String base = "players." + player.getUniqueId();
        for (PlayerTask task : tasks.values()) {
            long refreshAt = playerConfig.getLong(base + ".refresh-at." + task.id(), 0L);
            if (refreshAt > System.currentTimeMillis() && refreshAt < next) next = refreshAt;
        }
        return next == Long.MAX_VALUE ? 0L : next;
    }

    public String getNextRefreshRemaining(Player player) {
        long next = getNextRefreshMillis(player);
        if (next <= 0L) return "none";
        return String.valueOf(Math.max(0L, (next - System.currentTimeMillis() + 999L) / 1000L));
    }

    public String getNextRefreshDate(Player player) {
        long next = getNextRefreshMillis(player);
        if (next <= 0L) return "none";
        return DATE_FORMAT.format(Instant.ofEpochMilli(next).atZone(ZoneId.systemDefault()));
    }
