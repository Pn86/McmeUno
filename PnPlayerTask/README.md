...
- `%pnplayertask_tasktime_任务ID%`：指定任务距离下一次刷新剩余秒数；任务不存在或没有可刷新时间时返回 `none`。
- `%pnplayertask_tasktimeend_任务ID%`：指定任务下一次刷新的日期时间；任务不存在或没有可刷新时间时返回 `none`。

例如：`%pnplayertask_tasktime_TestTask%`、`%pnplayertask_tasktimeend_TestTask%`。
