...
## 2. 指令说明
### 管理员指令（`/pnlv`）
- `/pnlv reload`：重载全部配置（config/message/exp/gift/gui）。
...
- 菜单禁止取出、放入、拖拽物品：
  ...
  - 仅处理顶部菜单按钮点击。
- 奖励领取防重复：
  - 按 `last-claimed-level` 判断是否已领。
  - 单级领取会补领中间未领等级并更新记录。
  - 一键领取按区间批量发放后一次性推进记录，防止刷奖励。
...
- `level-exp.default`：默认升级所需经验。
- `level-exp.increment`：每级递增经验。
- `level-exp.custom.<level>`：某级自定义升级需求（优先级最高）。
...
- `gift.return-position`：`left_bottom` 或 `middle_bottom`。
- `gift.buttons.prev/next/claim-all`：分页与一键领取按钮文本。
- `gift.level-item.claimed/claimable/locked`：三种状态的材质、名称、Lore。
- `top.title`：排行榜标题。
- `top.return-position`：排行榜返回按钮位置。
- `top.item`：排行按钮格式。
