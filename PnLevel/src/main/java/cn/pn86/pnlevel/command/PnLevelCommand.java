    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (args[0].toLowerCase(Locale.ROOT)) {
            case "reload" -> {
                ...
                if (plugin.getLastReloadWarningCount() > 0) {
                    sender.sendMessage(ColorUtil.component("&e[PnLevel] 重载完成，但检测到 " + plugin.getLastReloadWarningCount() + " 个配置警告，请查看后台日志。"));
                }
            }
            case "list" -> {
            ...
            case "set", "add", "remove" -> {
                ...
                Player onlineTarget = Bukkit.getPlayerExact(data.getLastName());
                if (type.equals("exp")) value = Math.max(0, value);
                if (args[0].equalsIgnoreCase("set")) {
                    if (type.equals("exp")) data.setExp(value); else data.setLevel(Math.min(plugin.getMaxLevel(), value));
                } else if (args[0].equalsIgnoreCase("add")) {
                    if (type.equals("exp")) data.setExp(data.getExp() + value); else data.setLevel(Math.min(plugin.getMaxLevel(), data.getLevel() + value));
                } else {
                    if (type.equals("exp")) data.setExp(Math.max(0, data.getExp() - value)); else data.setLevel(Math.max(1, data.getLevel() - value));
                }
                if (onlineTarget != null && type.equals("exp")) {
                    plugin.getLevelManager().processLevelUps(data, onlineTarget, true);
                }
