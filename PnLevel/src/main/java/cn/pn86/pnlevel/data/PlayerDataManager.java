...
public class PlayerDataManager {
        ...
        for (String key : players.getKeys(false)) {
            try {
                ...
                PlayerLevelData data = new PlayerLevelData(
                        ...
                        section.getInt("exp", plugin.getInitialExp())
                );
                data.setLastClaimedLevel(section.getInt("last-claimed-level", 0));
                List<Integer> claimed = section.getIntegerList("claimed-levels");
                data.getClaimedLevels().addAll(claimed);
                if (data.getClaimedLevels().isEmpty() && data.getLastClaimedLevel() > 0) {
                    for (int i = 1; i <= data.getLastClaimedLevel(); i++) {
                        data.getClaimedLevels().add(i);
                    }
                }
    ...
    public void save() {
        ...
        for (PlayerLevelData data : cache.values()) {
            ...
            yaml.set(path + ".claimed-levels", new ArrayList<>(data.getClaimedLevels()));
