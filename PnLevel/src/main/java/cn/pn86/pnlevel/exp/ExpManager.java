...
public class ExpManager {
    ...
    public void loadRules() {
        ...
        for (String key : exp.getKeys(false)) {
            ...
            try {
                ...
                plugin.getLogger().warning("[PnLevel] exp.yml rule '" + key + "' has invalid type, skipped.");
                continue;
            }

            List<String> values = new ArrayList<>(section.getStringList("int"));
            if (values.isEmpty()) {
                String single = section.getString("int", "");
                if (!single.isBlank()) {
                    values.add(single);
                }
            }
            values.replaceAll(v -> v == null ? "" : v.trim());

            int upExp = parsePositiveInt(section, "upexp", 0);
            if (upExp <= 0) {
                plugin.getLogger().warning("[PnLevel] exp.yml rule '" + key + "' upexp <= 0, this rule will not grant EXP.");
            }
    ...
    private int parsePositiveInt(ConfigurationSection section, String path, int fallback) {
