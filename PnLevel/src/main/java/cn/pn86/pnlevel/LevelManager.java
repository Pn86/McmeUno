...
public class LevelManager {
    ...
    public int getRequiredExpForLevel(int currentLevel) {
        // fixed mode: all levels require same exp (user requested behavior)
    ...
    public void addExp(PlayerLevelData data, Player player, int amount, boolean sendMessage) {
        ...
        while (data.getLevel() < plugin.getMaxLevel()) {
            ...
            leveled = true;
            player.sendMessage(ColorUtil.component(plugin.msg("level-up").replace("%level%", String.valueOf(data.getLevel()))));
            player.sendMessage(ColorUtil.component(plugin.msg("gift-available").replace("%level%", String.valueOf(data.getLevel()))));
        }
