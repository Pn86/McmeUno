...
public class LevelManager {
    ...
    public int getRequiredExpForLevel(int currentLevel) {
        int fixed = plugin.getConfig().getInt("level-exp.fixed", -1);
        if (fixed > 0) {
            return fixed;
        }
    ...
    public void addExp(PlayerLevelData data, Player player, int amount, boolean sendMessage) {
        ...
        processLevelUps(data, player, true);

        if (sendMessage && player != null) {
            plugin.sendLang(player, "exp-add", "%exp%", String.valueOf(amount));
            plugin.sendLang(player, "exp-now",
                    "%exp%", String.valueOf(data.getExp()),
                    "%need%", String.valueOf(getRequiredExpForLevel(data.getLevel())));
        }
    }

    public int processLevelUps(PlayerLevelData data, Player player, boolean notify) {
        int up = 0;
        while (data.getLevel() < plugin.getMaxLevel()) {
            ...
            if (notify && player != null) {
                plugin.sendLang(player, "level-up", "%level%", String.valueOf(data.getLevel()));
                plugin.sendLang(player, "gift-available", "%level%", String.valueOf(data.getLevel()));
            }
        }
