...
PnWarp 是一个 Paper 1.21.1 的简单地标（Warp）插件，支持玩家创建自己的地标、GUI 菜单翻页浏览、延迟传送倒计时、数据持久化，以及 PlaceholderAPI 占位符。
...
- `/addwarp [地标名] [地标简介] [地标图标(MC物品ID)]`
  - 在当前位置创建自己的地标。
  - 地标名、地标简介都支持 `&` 颜色字符（例如 `&a家`、`&b传送点`）。
- `/remwarp [地标名]`
  - 删除自己的地标（管理员可删除任意地标）。
- `/warps`
  - 打开地标菜单（6 排箱子，底部一排按钮/玻璃板）。
- `/gowarp [地标名]`
  - 传送到地标（支持等待时间、详细倒计时、移动取消）。
...
- `allowed-set-warp-worlds`：允许玩家创建地标的世界名单（未列出的世界不能 `/addwarp`）。
- `teleport.wait-seconds`：传送等待秒数（0 为秒传）。
- `teleport.*-sound`：开始/取消/成功音效。
- `messages.*`：所有消息文本（含倒计时提示）。
